...
class SecurityAssessment():
    ...
    def run_trivy():
        ...
        sub = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def run_kube_bench(cluster_name):
        ...
        runJobSubproc = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        ...
        while True:
            jobWaitSubproc = subprocess.run(jobWaitCmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        ...
        getPodSubproc = subprocess.run(getPodCmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        # decoding adds newline or blank spaces - attempt to trim them
        kubebenchPodName = str(getPodSubproc.stdout.decode('utf-8')).replace('\n', '').replace(' ', '')

        # Pull logs from Job - this is the actual results of the job
        getLogsCmd = f'kubectl logs {kubebenchPodName}'
        getLogsSubproc = subprocess.run(getLogsCmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        ...
        kubeBenchResultRegex = re.compile('^\[.*\..*\..*')
        ...
        deleteKubebenchJobSubproc = subprocess.run(deleteKubebenchJobCmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
